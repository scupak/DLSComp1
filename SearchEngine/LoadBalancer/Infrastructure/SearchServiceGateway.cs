...
public class SearchServiceGateway : IServiceGateway<SearchResult>
{
    ...
    public async Task<SearchResult> Get(string serviceName, string parameters)
    {
        ...
        if (response.StatusCode == HttpStatusCode.OK && response.Content != null) return Newtonsoft.Json.JsonConvert.DeserializeObject<SearchResult>(response.Content) ?? throw new ArgumentException();
